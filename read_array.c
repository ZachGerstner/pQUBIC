...
{
    const continuous *da = (continuous *)a;
    const continuous *db = (continuous *)b;
...
{
	continuous** result = (continuous **)malloc(sizeof(continuous *) * rr);
	int i;
	/*AllocArray(result, rr);*/
	for (i = 0; i < rr; i++)
		result[i] = (continuous *)malloc(sizeof(continuous *) * cc);
...
{
	discrete** result = (discrete **)malloc(sizeof(discrete *) * rr);
	int i;
	/*AllocArray(result, rr);*/
	for (i = 0; i < rr; i++)
		result[i] = (discrete *)malloc(sizeof(discrete *) * cc);
		/*AllocArray(result[i], cc);*/
	return result;
}
...
void read_list (FILE* fp)
{
	...
	/*update the sub_list*/
	sublist = (bool *)malloc(sizeof(bool *) * rows);
...
void init_dis()
{
	...
	symbols = (discrete *)malloc(sizeof(discrete *) * USHRT_MAX);
